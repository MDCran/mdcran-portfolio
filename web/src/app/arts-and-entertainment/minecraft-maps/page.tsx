...
export default function MinecraftMapsPage() {
  ...
  const { data: clients = [] } = useSWR<Client[]>("/api/data/clients", fetcher, { revalidateOnFocus: false });
  const [query, setQuery] = useState("");
  const [statusFilter, setStatusFilter] = useState<ProjectStatus | "all">("all");
  const [adv, setAdv] = useState<AdvancedFilters>(EMPTY_ADVANCED);
  const [cols, setCols] = useGridCols("grid_cols_minecraft-maps");
