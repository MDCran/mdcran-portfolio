...
import RizzThemeEditor from "./RizzThemeEditor";
...
export default function RizzSettings({ content, onChange }: { content: SiteContent; onChange: Dispatch<SetStateAction<SiteContent>> }) {
  ...
  return <section className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 sm:p-6">
    <div className="mb-6"><h2 className="font-nord text-xl text-white">Personalize your invitation</h2><p className="mt-2 text-sm text-white/60">Style the rizz page, tailor the questions, and manage your secret pages. Save below to apply changes.</p></div>
    <fieldset disabled={saving} className="space-y-6">
      ...
      <p className="text-xs text-white/60">Leave the name blank for a nameless invitation. No sample name is inserted.</p>
      ...
      <RizzThemeEditor config={config} name={content.rizzTargetName}
        value={content.rizzThemeCustomizations?.[config.theme] ?? {}}
        onChange={value => update({ rizzThemeCustomizations: { ...content.rizzThemeCustomizations, [config.theme]: value } })} />
